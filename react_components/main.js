...
var TutorialList = React.createClass({
  render: function() {
        var tutorialNodes = this.props.data.map(function(tutorial, index) {
          return (
            <Tutorial author={tutorial.author} titleTuto={tutorial.titleTuto} dateTuto={tutorial.dateTuto} key={index}>{tutorial.text}</Tutorial>
            );
              });
        return (
    ...
    render: function() {
        return (
            <div className="blog-post">
                <h2 className="blog-post-title tutorialAuthor">{this.props.titleTuto}</h2>
                <span dangerouslySetInnerHTML={this.rawMarkup()} />
                <p className="blog-post-meta"> {this.props.dateTuto} par <a href="#">{this.props.author}</a></p>
            </div>
        );
    }
});
